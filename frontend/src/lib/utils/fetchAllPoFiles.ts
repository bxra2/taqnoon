...
import { parseTxTFiles } from '$lib/utils/parseTxTFiles';
import { CinnamonUrls, GnomeUrls, KDEUrls, ObsidianUrls, AudacityUrls } from '../data/localizationUrls';


const poSources = [
  ...GnomeUrls, ...CinnamonUrls, ...KDEUrls, ...ObsidianUrls, ...AudacityUrls
]
...
export async function fetchAndParseAllPoFiles({ concurrency = 4 } = {}) {
  ...
  for (const item of settled) {
    if (item?.status === 'fulfilled') {
      ...
      try {
        const parsed = src.type === 'txt' ? parseTxTFiles(text) : parsePo(text);
